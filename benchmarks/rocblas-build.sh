#!/bin/bash
# rocblas-build.sh — rocBLAS + Tensile compile script
# GPU: RX 9060 XT (gfx1200 / RDNA4)
# TheRock: 7.13, LLVM: 23.0.0
# Build: /data/ROCm/rocBLAS/build
# Target: /opt/rocm/core-7.13
...
configure() {
    ...
    mkdir -p "$BUILD_DIR"
    cd "$BUILD_DIR"
    # 强制清除旧 cmake 缓存中的 TENSILE_VERSION
    ...
    cmake "$ROCBLAS_DIR" \
        ...
        -DTensile_TEST_LOCAL_PATH=/data/ROCm/Tensile \
        -DTENSILE_VERSION=4.44.0 \
        -DCACHE_TENSILE_VERSION=4.44.0 \
